 ...
 */
export async function getAllFluencyProgress(studentUid: string): Promise<MathFluencyProgress[]> {
...
export async function bulkAddStudentsToFluencyProgram(
  ...
  console.log(
    `✅ Bulk add complete: ${successCount} added, ${alreadyInCount} already in program, ${failedCount} failed`,
  )
...
export async function createInitialFluencyProgress(
  ...
  try {
    const distribution = {
      ...
      total:
        problemBanks.doesNotKnow.length +
    ...
    operationSubLevels.forEach((config) => {
      ...
      const { proficiencyPercentage: subLevelProf } = calculateSubLevelProficiency(
        allProblems,
        config.id,
      )
...
export async function updateDailyPracticeLimit(
  ...
  try {
    ...
    console.log(
      `✅ Updated practice limit for ${studentUid} to ${limit === 999 ? 'unlimited' : limit + ' per day'}`,
    )
  } catch (error) {
...
export async function updateProgressAfterSession(
  ...
  try {
    ...
    // This ensures we don't lose problems that weren't practiced in this session
    const allExistingProblems: ProblemProgress[] = [
    ...
    // Update problems that were practiced in this session
    sessionResults.allProblems.forEach((sessionProblem) => {
      const result = allResults[sessionProblem.problemId]
      if (result) {
        // Get existing problem or use session problem as base
        const existingProblem = existingProblemsMap.get(sessionProblem.problemId) || sessionProblem
        const updatedProblem = updateProblemProficiencyLevel(existingProblem, result)
        existingProblemsMap.set(sessionProblem.problemId, updatedProblem)
      } else {
        // Problem was in session but no result - use existing if available, otherwise add session version
        if (!existingProblemsMap.has(sessionProblem.problemId)) {
          existingProblemsMap.set(sessionProblem.problemId, sessionProblem)
        }
      }
    })
    ...
    if (progress.currentSubLevel && subLevelProgress) {
      ...
      if (currentSubLevelData) {
        ...
        const subLevelProblems = filterProblemsBySubLevel(updatedProblems, progress.currentSubLevel)

        const subLevelProficiency =
          subLevelProblems.length > 0
            ? Math.round(
                (subLevelProblems.filter(
                  (p) =>
                    p.proficiencyLevel === 'approaching' ||
                    p.proficiencyLevel === 'proficient' ||
                    p.proficiencyLevel === 'mastered',
                ).length /
                  subLevelProblems.length) *
                  100,
              )
            : 0
...
function updateProblemProficiencyLevel(
  ...
  if (result.responseTime) {
    problem.averageResponseTime =
      problem.responseTimes.reduce((sum, t) => sum + t, 0) / problem.responseTimes.length
...
export async function updateProblemInProgress(
  ...
  try {
    ...
    const avgTime = calculateAverage(problem.last5Attempts.map((a) => a.responseTime))
    ...
    problem.proficiencyCalculation = {
      ...
      confidenceLevel:
        problem.totalAttempts < 5 ? 'low' : problem.totalAttempts < 10 ? 'medium' : 'high',
    }
    ...
    if (newProficiencyLevel !== problem.proficiencyLevel) {
      ...
      if (currentBankName) {
        progress.problemBanks[currentBankName] = progress.problemBanks[currentBankName].filter(
    ...
    progress.proficiencyPercentage = Math.round(
      ((progress.proficiencyDistribution.approaching +
        ...
        progress.proficiencyDistribution.total) *
        100,
    )

    progress.masteryPercentage = Math.round(
      ((progress.proficiencyDistribution.proficient + progress.proficiencyDistribution.mastered) /
        progress.proficiencyDistribution.total) *
        100,
    )
  ...
  try {
    ...
    const template = {
      ...
      problems: problems.map((problem, index) => {
        ...
        const displayText =
          problem.displayText || `${problem.num1} ${getOpSymbol(operation)} ${problem.num2}`
 ...
 */
export async function getPaperAssessmentTemplate(templateId: string): Promise<any | null> {
  try {
 ...
 */
export async function getPendingPaperAssessments(teacherUid: string): Promise<any[]> {
  try {
    const q = query(
      collection(db, 'mathFluencyPaperAssessments'),
...
export function findProblemInBanks(
  ...
  for (const [bankName, bank] of Object.entries(problemBanks) as [
    keyof ProblemBanks,
    ProblemProgress[],
  ][]) {
...
export function moveProblemToBucket(
  ...
  problemBanks[bankName] = problemBanks[bankName].filter(
    (p: ProblemProgress) => p.problemId !== problemId,
  )

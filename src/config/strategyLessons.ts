...
const making5Lesson: StrategyLesson = {
  ...
  nextSteps: 'In your next practice session, try to use the Making 5 strategy when you see problems like 4+1 or 2+3!'
};
...
const making10Lesson: StrategyLesson = {
  ...
  operation: 'addition',
  requiredAfterSession: 6,  // Trigger after 6 practice sessions
  prerequisites: ['making-5'],
  
  overview: {
    what: 'Finding two numbers that add up to exactly 10. This is THE most powerful strategy for addition!',
    
    why: 'Making 10 is the key to solving hard problems like 8+7 or 9+6 quickly. Once you make 10, adding the rest is easy (10+5=15).',
    
    when: 'Use this for problems where the sum is greater than 10 (bridging problems like 8+7, 9+6).',
    
    examples: [
      '8 + 7: Make 10 first (8+2=10), then add the rest (10+5=15)',
      '9 + 6: Make 10 first (9+1=10), then add the rest (10+5=15)',
      '7 + 8: Make 10 first (7+3=10), then add the rest (10+5=15)'
    ],
  ...
  practice: {
    type: 'scaffolded',
    problems: generateMaking10Problems(),
    successCriteria: {
      minCorrect: 10,
  ...
  completionMessage: 'Excellent! You learned Making 10 - the most important addition strategy!',
  nextSteps: 'Tomorrow you\'ll practice "bridging" problems (8+7, 9+6) using the Making 10 strategy. You\'ll get faster with practice!'
};
...
const decomposingLesson: StrategyLesson = {
  ...
  nextSteps: 'You\'ll practice bridging problems with the ten frame visual available if you need it. Over time, you\'ll be able to do it without the visual!'
};
...
function generateMaking5Problems(): LessonProblem[] {
  ...
  [[4, 1], [3, 2], [1, 4], [2, 3], [5, 0]].forEach(([num1, num2], index) => {
    problems.push({
...
function generateMaking10Problems(): LessonProblem[] {
  const problems: LessonProblem[] = [];
  
  // Part A: Partners of 10 (missing number format)
  const partners = [
    [9, 1], [8, 2], [7, 3], [6, 4], [5, 5],
    [1, 9], [2, 8], [3, 7], [4, 6]
  ];
  ...
  // Part B: Scaffolded bridging problems (step-by-step)
  const bridgingProblems = [
    [8, 7, 15],  // 8+7=15
    [9, 6, 15],  // 9+6=15
    [7, 8, 15],  // 7+8=15
    [6, 9, 15],  // 6+9=15
  ...
  bridgingProblems.forEach(([num1, num2, sum], index) => {
    const needed = 10 - num1;
    const remaining = num2 - needed;
    
    problems.push({
      id: `making10-bridge-${index}`,
      ...
      feedbackCorrect: `✅ Excellent! You used Making 10 to solve ${num1} + ${num2} = ${sum}`,

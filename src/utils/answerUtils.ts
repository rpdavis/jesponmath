...
export const normalizeAnswer = (answer: string): string => {
  ...
  return (
    answer
      .toLowerCase()
      .trim()
      // Remove ALL spaces (handles "4 x + 32" → "4x+32")
      .replace(/\s+/g, '')
      // Normalize fraction spacing (1/2 already normalized by removing spaces)
      .replace(/\s*\/\s*/g, '/')
      ...
      .replace(/÷/g, '/')
      // Spaces already removed, so this catches any edge cases
      .replace(/\s*([+\-*/^_=<>])\s*/g, '$1')
  )
}

/**
 * Normalize variables in algebraic expressions
 * Replaces all single-letter variables with 'x' for comparison
 * This allows 4c+5=6 to match 4x+5=6
 */
export const normalizeVariables = (answer: string): string => {
  ...
  // Matches: standalone letters, letters with coefficients (like 4c), letters with operators
  // Avoids: letters inside words, function names
  return (
    answer
      // First normalize to lowercase for consistency
      .toLowerCase()
      // Replace patterns like: 4c, c+5, -c, c/2, c*3, c^2, c=, (c), |c|
      .replace(/(\d)([a-z])(?=[+\-*/^=\s)]|$)/g, '$1x') // 4c -> 4x
      .replace(/([+\-*/^=(|])([a-z])(?=[+\-*/^=\s)]|$)/g, '$1x') // +c -> +x, (c -> (x
      .replace(/^([a-z])(?=[+\-*/^=\s)]|$)/g, 'x') // c at start -> x
      .replace(/\s([a-z])(?=[+\-*/^=\s)]|$)/g, ' x')
  ) // space c -> space x
}
 ...
 * Now also handles LaTeX/KaTeX expressions and optional variable normalization
 */
export const areAnswersEquivalent = (
  ...
  const hasLatex1 =
    ...
    studentAnswer.includes('\\frac') ||
    studentAnswer.includes('katex')
  const hasLatex2 =
    correctAnswer.includes('$') ||
    correctAnswer.includes('\\frac') ||
    correctAnswer.includes('katex')
  ...
  let normalizedStudent = normalizeAnswer(studentPlainText)
  let normalizedCorrect = normalizeAnswer(correctAnswer)

  // Apply variable normalization if requested
  if (acceptAnyVariable) {
    normalizedStudent = normalizeVariables(normalizedStudent)
    normalizedCorrect = normalizeVariables(normalizedCorrect)
  }
 ...
 * Now also handles LaTeX/KaTeX expressions and optional variable normalization
 */
export const areAnswersEquivalentBasic = (
  studentAnswer: string,
  correctAnswer: string,
  acceptAnyVariable = false,
): boolean => {

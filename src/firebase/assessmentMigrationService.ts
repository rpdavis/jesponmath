...
const regradeQuestionResponse = (
  ...
  if (updatedQuestion.questionType === 'multiple-choice') {
    ...
    if (
      !isCorrect &&
      updatedQuestion.acceptableAnswers &&
      updatedQuestion.acceptableAnswers.length > 0
    ) {
    ...
    if (typeof userAnswer === 'string' && typeof updatedQuestion.correctAnswer === 'string') {
      ...
      isCorrect =
        ...
        updatedQuestion.correctAnswer?.toString().trim().toLowerCase()
    }
  }
    ...
    for (const result of existingResults) {
      ...
      const updatedResponses =
        result.responses?.map((response) => {
          const updatedQuestion = updatedAssessment.questions?.find(
          ...
          if (gradeResult.wasChanged) {
            ...
            questionsChanged.push({
              questionId: response.questionId,
              questionText:
                updatedQuestion.questionText?.substring(0, 50) + '...' || 'Question text',
              oldPoints: response.pointsEarned || 0,
          ...
          // Only add regraded fields if they have values
          if (gradeResult.wasChanged) {
            updatedResponse.regraded = true
            updatedResponse.regradedAt = new Date()
            updatedResponse.regradedBy = performedBy
          } else if (response.regradedAt !== undefined) {
            updatedResponse.regradedAt = response.regradedAt
          }
          if (response.regradedBy !== undefined) {
            updatedResponse.regradedBy = response.regradedBy
          }
 ...
 */
export const hasExistingResults = async (
  assessmentId: string,
): Promise<{
  hasResults: boolean
  resultCount: number

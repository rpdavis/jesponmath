...
import { loadAcademicPeriodSettings } from './appSettingsService'

// ==================== HELPER FUNCTIONS ====================

// Cache for academic year settings to avoid repeated async calls
let cachedAcademicYear: AcademicYear | null = null
let cacheLoadPromise: Promise<AcademicYear | null> | null = null

/**
 * Load academic year settings (cached)
 */
async function getCachedAcademicYear(): Promise<AcademicYear | null> {
  ...
  cacheLoadPromise = loadAcademicPeriodSettings().then((config) => {
    if (config) {
      cachedAcademicYear = config
    }
    return config
  })
 ...
 * Uses cached settings if available, falls back to defaults
 */
export function getAutoDetectedAcademicPeriod(): string {
  try {
    // Try to use cached settings first (if already loaded)
    if (cachedAcademicYear) {
      const currentPeriod = getCurrentPeriod(cachedAcademicYear)
      if (currentPeriod) {
        console.log(
          `📅 Auto-detected academic period from cached settings: ${currentPeriod.id} (${currentPeriod.name})`,
        )
        return currentPeriod.id
      }
    }

    // Fallback to default generation if no cached settings
    ...
    if (currentPeriod) {
      console.log(
        `📅 Auto-detected academic period (default): ${currentPeriod.id} (${currentPeriod.name})`,
      )
      return currentPeriod.id
    }
...
/**
 * Initialize academic period cache (call this on app startup)
 */
export async function initializeAcademicPeriodCache(): Promise<void> {
  try {
    const config = await getCachedAcademicYear()
    if (config) {
      console.log(`✅ Academic period cache initialized: ${config.periodType} for ${config.year}`)
    }
  } catch (error) {
    console.error('❌ Error initializing academic period cache:', error)
  }
}

/**
 * Clear the academic period cache (useful after admin updates settings)
 */
export function clearAcademicPeriodCache(): void {
...
export async function bulkAssignAssessment(
  ...
  try {
    ...
    console.log(
      `🔍 Found ${alreadyAssigned.length} existing assignments for assessment ${assessmentId}`,
    )
    ...
    console.log(
      `✅ Assessment assigned to ${studentsToAssign.length} students in ${academicPeriod} (${alreadyAssigned.length} were already assigned)`,
    )
...
export async function bulkUpdateAssignmentTeacher(
  ...
  try {
    ...
    // Execute all batches
    console.log(
      `🔄 Executing ${batches.length} batches to update ${updatedCount} assignments and assessment...`,
    )
    for (let i = 0; i < batches.length; i++) {
      try {
        await batches[i].commit()
    ...
    console.log(
      `✅ Updated ${updatedCount} assignment(s) and assessment to teacher ${newTeacherUid}`,
    )
    return updatedCount
  } catch (error) {
